...
:call_installers
  ...
  REM Install OWASP Zap (https://www.zaproxy.org/download/)
  call :log "Installing ZAP For Windows (ZAP_2_16_0_windows)"
  copy "%COMMON_DIR%\ZAP_2_16_0_windows.exe" %DEVDIR%
  "%DEVDIR%\ZAP_2_16_0_windows.exe" -q -dir "C:\Program Files\ZAP" -overwrite -nofilefailures > nul 2>&1
  if %errorlevel% neq 0 call :log "ZAP installation failed with error code %errorlevel%"

  REM Curl demo.zip spring project for testing
  call :log "Spring Init with curl"
  curl -o %DEVDIR%\demo.zip "https://start.spring.io/starter.zip?type=maven-project&language=java&bootVersion=3.4.2&groupId=com.example&artifactId=demo&name=demo&packageName=com.example.demo&javaVersion=17&dependencies=web"
  "C:\Program Files\7-Zip\7z.exe" x "%DEVDIR%\demo.zip" -o"%DEVDIR%"\demo -y > nul 2>&1
  ...
  set "NEW_PATH=!PATH!;!JAVA_HOME!\bin;!ANT_HOME!\bin;!MAVEN_HOME!\bin;C:\Program Files\7-Zip;C:\Program Files\Git\bin;C:\Program Files\Notepad++;C:\Development\eclipse-jee-2024-12-R-win32-x86_64\eclipse;C:\Program Files\ZAP"
  set PATH=!NEW_PATH!
