...
:call_installers
  ...
  winget install -e --id PostgreSQL.PostgreSQL.17 -h --scope machine --accept-source-agreements --silent > nul 2>&1
  if %errorlevel% neq 0 call :log "PostgreSQL installation failed with error code %errorlevel%"
  call :log "Stopping PostgreSQL Service"
  net stop postgresql-x64-17
  sc config postgresql-x64-17 start= demand

  REM Set PostgreSQL PGDATA Environment Variable
  set PGDATA=C:\sandbox\data\postgresql_data
  setx PGDATA C:\sandbox\data\postgresql_data /m
  call :log "PGDATA=%PGDATA%"
  ...
  REM Shortcut for Starting PostgreSQL Service
  call :log "Creating shortcut for Start PostgreSQL"
  powershell -ExecutionPolicy Bypass -File "%COMMON_DIR%\create_desktop_shortcut.ps1" -ShortcutName "Start PostgreSQL" -TargetPath "C:\Program Files\PostgreSQL\17\bin\pg_ctl.exe" -Arguments "start -D %PGDATA%" -WorkingDirectory "C:\Program Files\PostgreSQL\17\bin"

  REM Shortcut for Stopping PostgreSQL Service
  call :log "Creating shortcut for Stop PostgreSQL"
  powershell -ExecutionPolicy Bypass -File "%COMMON_DIR%\create_desktop_shortcut.ps1" -ShortcutName "Stop PostgreSQL" -TargetPath "C:\Program Files\PostgreSQL\17\bin\pg_ctl.exe" -Arguments "stop -D %PGDATA%" -WorkingDirectory "C:\Program Files\PostgreSQL\17\bin"

  REM Shortcut for Opening PostgreSQL Command Line (psql)
  call :log "Creating shortcut for PostgreSQL Command Line"
  powershell -ExecutionPolicy Bypass -File "%COMMON_DIR%\create_desktop_shortcut.ps1" -ShortcutName "PostgreSQL CLI (psql)" -TargetPath "C:\Program Files\PostgreSQL\17\bin\psql.exe" -Arguments "-U postgres" -WorkingDirectory "C:\Program Files\PostgreSQL\17\bin"

  REM Shortcut for Opening pgAdmin 4
  call :log "Creating shortcut for pgAdmin 4"
  powershell -ExecutionPolicy Bypass -File "%COMMON_DIR%\create_desktop_shortcut.ps1" -ShortcutName "pgAdmin 4" -TargetPath "C:\Program Files\PostgreSQL\17\pgAdmin 4\runtime\pgAdmin4.exe" -WorkingDirectory "C:\Program Files\PostgreSQL\17\pgAdmin 4\runtime"

  REM Shortcut for Restarting PostgreSQL Service
  call :log "Creating shortcut for Restart PostgreSQL"
  powershell -ExecutionPolicy Bypass -File "%COMMON_DIR%\create_desktop_shortcut.ps1" -ShortcutName "Restart PostgreSQL" -TargetPath "C:\Program Files\PostgreSQL\17\bin\pg_ctl.exe" -Arguments "restart -D %PGDATA%" -WorkingDirectory "C:\Program Files\PostgreSQL\17\bin"

  REM Set PATH

  set "NEW_PATH=!PATH!;!JAVA_HOME!\bin;!ANT_HOME!\bin;!MAVEN_HOME!\bin;C:\Program Files\7-Zip;C:\Program Files\Git\bin;C:\Program Files\Notepad++;C:\Development\eclipse-jee-2024-12-R-win32-x86_64\eclipse;C:\Program Files\PostgreSQL\17\bin"
  set PATH=!NEW_PATH!
